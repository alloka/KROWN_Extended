...
def build_reverse_program(
    ...
    if provenance_enabled:
        lines.append('// Per-column provenance inputs are declared in the Evidence section below')
        lines.append('')

    # Build a canonical parse relation name per unique (source, template) pair
    # so subjects that share an identical IRI template share one parse rule.
    _subj_template_to_parse_name: Dict[Tuple[str, str], str] = {}
    _subj_shared_parse_name: Dict[str, str] = {}  # pred_name -> canonical relation name
    for _subj in subject_rules.values():
        if not source_decls.get(_subj.source_name):
            continue
        _key = (_subj.source_name, _subj.term_expr)
        if _key not in _subj_template_to_parse_name:
            _subj_template_to_parse_name[_key] = f'Parse_{_subj.pred_name}'
        _subj_shared_parse_name[_subj.pred_name] = _subj_template_to_parse_name[_key]

    # Emit parsers — only needed for template-based (non-provenance) reconstruction
    if not provenance_enabled:
        _emitted_subj_parsers: Set[str] = set()
        for subj in subject_rules.values():
            src = source_decls.get(subj.source_name)
            if not src:
                continue
            canonical = _subj_shared_parse_name.get(subj.pred_name, f'Parse_{subj.pred_name}')
            if canonical in _emitted_subj_parsers:
                continue
            _emitted_subj_parsers.add(canonical)
    ...
    handled_sources = set()
    # cols ever emitted as direct (S/P/O/G) in any pattern
    direct_cols_by_source: Dict[str, Set[str]] = {}
    # cols ever seen as fallback across all patterns, with their position
    fallback_col_pos_by_source: Dict[str, Dict[str, int]] = {}
    # all cols referenced in any mapping template (including non-reversible)
    mapping_cols_by_source: Dict[str, Set[str]] = {}
    # (src, canonical_subj_parse, head_kind) → union of ProvContributor cols across all predicates of this TM
    tm_prov_info: Dict[Tuple[str, str, str], Set[str]] = {}
    ...
    for tp in triple_patterns:
        ...
        # accumulate all cols that appear in this pattern's mapping templates
        mapping_cols_by_source.setdefault(src_name, set()).update(subj_cols)
        ...
        if pred and pred.constant_iri is None and parser_can_recover(pred.parts):
            ...
            mapping_cols_by_source.setdefault(src_name, set()).update(pred_cols)
            if pred_cols:
        ...
        if obj:
            ...
            mapping_cols_by_source.setdefault(src_name, set()).update(obj_cols)
        ...
        if graph_rule:
            ...
            mapping_cols_by_source.setdefault(src_name, set()).update(graph_cols)
        ...
        if obj_subj and parser_can_recover(obj_subj.parts):
            ...
            if ref_cols:
                mapping_cols_by_source.setdefault(src_name, set()).update(ref_cols)
        ...
        def build_parse_atoms_with_needed(needed_vars: Set[str]) -> List[str]:
            ...
            if subj_parse_vars:
                ...
                _canon_subj = _subj_shared_parse_name.get(subj.pred_name, f'Parse_{subj.pred_name}')
                atoms.append(f'{_canon_subj}(s, {", ".join(subj_args)})')
            if pred_parse_name:
        ...
        provenance_cols: List[Tuple[str, str, int, str]] = []
        if provenance_enabled:
            src_pos = {c: i for i, c in enumerate(source.columns)}
            for c, v in subj_var_for_source.items():
                provenance_cols.append((c, v, src_pos.get(c, -1), 's'))
            for c in pred_cols:
                provenance_cols.append((c, c, src_pos.get(c, -1), 'p'))
            for c in obj_cols:
                provenance_cols.append((c, c, src_pos.get(c, -1), 'o'))
            if graph_parse_vars:
                for c in graph_cols:
                    provenance_cols.append((c, c, src_pos.get(c, -1), 'g'))
        ...
        # snapshot direct-recoverable cols before fallback aug; Evidence head uses "" for the rest
        pre_fallback_col_var: Set[str] = set(col_var_for_source.keys())
        ...
        if provenance_enabled:
            for c in source.columns:
                ...
                provenance_cols.append((c, val_var, src_pos.get(c, -1), 'fallback'))
        ...
        # Evidence head: fallback cols use "" — ProvContributor recovery tracked separately via Prov_Column_Fallback
        head_args = []
        is_complete = True
        for c in source.columns:
            if c in pre_fallback_col_var:
        ...
        for triple_p in triple_p_terms:
            # ProvContributor-based Evidence: accumulate per-TM cols for cross-predicate rule emitted after loop
            if provenance_enabled:
                _canon_subj_for_tm = _subj_shared_parse_name.get(subj.pred_name, f'Parse_{subj.pred_name}')
                tm_prov_info.setdefault((src_name, _canon_subj_for_tm, tp.head_kind), set()).update(
                    unique_preserve_order(list(subj_cols) + pred_cols + obj_cols)
                )
            ...
            for triple_o in triple_o_terms:
                ...
                # ProvContributor joins are in Prov_Column_Fallback rules only, not in Evidence
    ...
    if provenance_enabled and tm_prov_info:
        # Group unique col-sets by (source, head_kind); emit per-column views + one Evidence rule per source.
        per_src_sets: Dict[Tuple[str, str], List[List[str]]] = {}
        for (src_name, canon_subj, head_kind), cols in tm_prov_info.items():
            per_src_sets.setdefault((src_name, head_kind), []).append(sorted(cols))

        for (src_name, head_kind), col_set_list in sorted(per_src_sets.items()):
            source = source_decls.get(src_name)
            if not source:
                continue

            # Union of all columns covered by any TM for this source
            all_covered: Set[str] = set()
            for cs in col_set_list:
                all_covered.update(cs)

            # Per-column inputs: forward program writes ProvCol_*.csv directly, no bridge step needed
            col_view_prefix = 'ProvCol'
            for c in sorted(all_covered):
                view_name = f'{col_view_prefix}_{src_name}_{c}'
                lines.append(f'.decl {view_name}(val:symbol, row_id:number)')
                lines.append(f'.input {view_name}(filename="{view_name}.csv", delimiter="\\t")')
            lines.append('')

            # Evidence rule: one ProvCol atom per column, all joined on row_id
            pvc: Dict[str, str] = {}
            pb: List[str] = []
            for c in sorted(all_covered):
                pvc[c] = f'cp_{c}'
                pb.append(f'{col_view_prefix}_{src_name}_{c}(cp_{c}, row_id)')

            prov_ev_idx = evidence_count_by_source.get(src_name, 0) + 1
            evidence_count_by_source[src_name] = prov_ev_idx
            prov_ev_name = f'Evidence_{src_name}_{prov_ev_idx}'
            lines.append(f'.decl {prov_ev_name}({", ".join(c + ":symbol" for c in source.columns)})')
            head2 = [pvc.get(c, '""') for c in source.columns]
            lines.append(f'{prov_ev_name}({", ".join(head2)}) :- {", ".join(pb)}.')
            lines.append('')
            evidence_by_source.setdefault(src_name, []).append(prov_ev_idx)
...
def build_forward_provenance_program(
    ...
    def pick_input_filename(rel_name: str) -> str:
        ...
        original_rel_candidate = None
        if rel_name.startswith('S_'):
            original_rel_candidate = f'{rel_name[2:]}.facts'

        if input_dir:
            if os.path.exists(os.path.join(input_dir, rel_candidate)):
                return rel_candidate
            if original_rel_candidate and os.path.exists(os.path.join(input_dir, original_rel_candidate)):
                return original_rel_candidate
            if lt_candidate and os.path.exists(os.path.join(input_dir, lt_candidate)):
                return lt_candidate

        if original_rel_candidate:
            return original_rel_candidate
    ...
    # Per-column output relations — each replaces a filtered slice of ProvContributor
    _prov_col_declared: Set[str] = set()
    _prov_col_all: List[str] = []

    # Emit one RowId helper per source relation using autoinc() for stable cross-TM join keys
    _rowid_emitted: Set[str] = set()
    for _tp in triple_patterns:
        _rsrc = find_source_for_triple_pattern(_tp, subject_rules, predicate_rules, object_rules)
        if not _rsrc or _rsrc in _rowid_emitted:
            continue
        _rsd = source_decls.get(_rsrc)
        if not _rsd:
            continue
        _rowid_emitted.add(_rsrc)
        _col_sig = ", ".join(c + ":symbol" for c in _rsd.columns)
        _col_args = ", ".join(_rsd.columns)
        lines.append(f'.decl RowId_{_rsrc}(row_id:number, {_col_sig})')
        lines.append(f'RowId_{_rsrc}(autoinc(), {_col_args}) :- {_rsrc}({_col_args}).')
        lines.append('')
    ...
    for tp in triple_patterns:
        ...
        if source_atom_args is not None:
            ...
            rowid_atom = f'RowId_{src_name}(row_id, {", ".join(source_atom_args)})'
            if contributor_source_atom in contributor_atoms:
                contributor_atoms[contributor_atoms.index(contributor_source_atom)] = rowid_atom
            elif rowid_atom not in contributor_atoms:
                contributor_atoms.append(rowid_atom)
        ...
        _row_id_var = 'row_id' if source_atom_args is not None else '0'
        for col, val_var, pos in contributors:
            _prov_rel = f'ProvCol_{src_name}_{col}'
            if _prov_rel not in _prov_col_declared:
                _prov_col_declared.add(_prov_rel)
                _prov_col_all.append(_prov_rel)
                lines.append(f'.decl {_prov_rel}(val:symbol, row_id:number)')
            lines.append(f'{_prov_rel}({val_var}, {_row_id_var}) :- {", ".join(contributor_atoms)}{target_guard}.')
    ...
    for _prov_rel in _prov_col_all:
        lines.append(f'.output {_prov_rel}(filename="{_prov_rel}.csv", delimiter="\\t")')
